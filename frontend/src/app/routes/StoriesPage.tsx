...
const STORIES: Story[] = [
  { id: 1, type: 'image', src: '/stories/1.png', bgColor: 'bg-black' },
  { id: 2, type: 'image', src: '/stories/2.png', bgColor: 'bg-black' },
  { id: 3, type: 'image', src: '/stories/3.png', bgColor: 'bg-black' },
  { id: 4, type: 'image', src: '/stories/4.png', bgColor: 'bg-black' },
  { id: 5, type: 'image', src: '/stories/5.png', bgColor: 'bg-black' },
  { id: 6, type: 'image', src: '/stories/6.png', bgColor: 'bg-black' },
  { id: 7, type: 'component', bgColor: 'bg-[#FE6601]' }
];
...
export function StoriesPage() {
  ...
  useEffect(() => {
    // Auto-advance logic: go to next story, but stop if it's the last one
  ...
  const handleTouch = (e: React.MouseEvent<HTMLDivElement> | React.TouchEvent<HTMLDivElement>) => {
    ...
    // Left third goes back, right two-thirds go forward (similar to Instagram)
  ...
  return (
    <div className={`h-[100dvh] w-full flex flex-col ${currentStory.bgColor} text-white relative transition-colors duration-300 overflow-hidden`}>
      <style>
        {`
      ...
      <div className="absolute top-0 left-0 w-full pt-4 px-2 flex gap-1 z-20">
        {STORIES.map((story, idx) => (
          <div key={story.id} className="h-1 flex-1 bg-white/30 rounded-full overflow-hidden">
            <div 
              // React key forces the div to completely remount when we switch to its index,
              // which ensures the CSS animation restarts from 0% every time.
      ...
      >
        {/* Images */}
        {STORIES.map((story, idx) => story.type === 'image' && (
          <img 
            key={story.id}
            src={story.src} 
            alt={`Story ${story.id}`} 
        ...
        {currentStory.type === 'component' && (
          <div className={`absolute top-0 left-0 w-full h-full flex flex-col items-center pt-[15vh] px-8 text-center z-10 bg-[#FE6601]`}>
            ...
            <div 
              ...
              onClick={(e) => e.stopPropagation()} // Prevent clicking links from triggering next story
            >
              <a 
